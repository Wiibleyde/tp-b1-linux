...
```bash
[nathan@storage ~]$ sudo vgcreate data /dev/sdb
  Volume group "data" successfully created
```
```bash
[nathan@storage ~]$ sudo lvcreate -L 1.5G data -n part_data
...
```bash
[nathan@storage ~]$ sudo mkfs -t ext4 /dev/data/part_data 
...
```bash
[nathan@storage ~]$ sudo mount /dev/data/part_data /mnt/storage/
```

```bash
[nathan@storage ~]$ cat /etc/fstab 
...
```
```bash
[nathan@storage ~]$ sudo umount /mnt/storage/
[nathan@storage ~]$ sudo mount -av
/                        : ignored
