...
```bash
[nathan@db ~]$ sudo dnf install mariadb-server
...
```bash
[nathan@db ~]$ sudo systemctl start mariadb
```

```bash
[nathan@db ~]$ sudo systemctl enable mariadb
...
```bash
[nathan@db ~]$ sudo ss -tulpn | grep 3306
tcp   LISTEN 0      80                 *:3306            *:*    users:(("mariadbd",pid=3475,fd=19))
```
```bash
[nathan@db ~]$ sudo firewall-cmd --add-port=3306/tcp --permanent
success
```

```bash
[nathan@db ~]$ sudo firewall-cmd --reload
success
```
🌞 **Processus liés à MariaDB**

```bash
[nathan@db ~]$ sudo ps -aux | grep mariadb
mysql       3475  0.0 11.2 1084832 88140 ?       Ssl  17:01   0:00 /usr/libexec/mariadbd --basedir=/usr
nathan      3565  0.0  0.2   6408  2140 pts/0    S+   17:03   0:00 grep --color=auto mariadb
```
